...
(defmacro oauth2-auto--query-case (&rest cases)
  "Extract and binds keys `symbols' from `query-alist'.
If all of them are present, respond with `msg' and runs `body'.  For format of
CASES see ‘oauth2-auto--httpd-filter’."
  `(cond
    ,@(mapcar (lambda (case)
